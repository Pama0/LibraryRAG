...
| `core/retrieval/retrieve.py` | 可插拔 Retriever（注入式）：`vector`=向量基线，`hybrid`=dense+BM25（jieba 分词、RRF 融合、scope 后过滤）；装配时按名字注入，不传="vector"；eval `VARIANTS` 以名字选择，量化召回侧增益 | Layer 0 检索数据源 |
