...
- pending_split（需要拆分）：判据=**一条检索 query 覆盖不全，必须扇出多个彼此独立的子查询**（这些子查询在检索前就能一次列全、可并行检索，互不依赖）。触发于以下二者之一：
  · 多主体（看问题文本）：显式含 ≥2 个并列主体，需各自检索再比较/综合，否则复合 query 会语义稀释、偏向其中之一。如「Vue和React的区别」「功能A和功能B」。
  · 广度分散（看 probe 形状）：单一大主题铺成多个互不重叠的子领域，单轮 top-k 覆盖不全，且末尾【知识库探测召回】显示命中**跨多个章节、无明显主导章**佐证。如「怎么优化MySQL」（索引/查询/配置/架构散在多章）。
  特征：答案需罗列/综合多个并列子项才完整，且子查询互相独立、不依赖彼此结果。
  与 other 的边界：若子查询之间有**依赖**（后一个要等前一个的答案，即多跳），不归这里，归 other。
...
- other（高难度/开放复杂问题）：**召回到了相关内容，但**需要【多跳依赖检索、跨主题综合多步推理，或开放设计/权衡比较】，单轮、甚至一次性并行扇出都答不全，必须多轮检索+推理逐步求解。
  特征二者之一：
  · 多跳依赖：子查询之间有依赖，后一跳的 query 要等前一跳的答案才写得出来（检索前列不全子查询）。如「MySQL 默认隔离级别会有哪些并发问题」——先查出默认级别是 RR，才能去查 RR 的并发问题。
  · 开放综合/权衡：要综合多处证据分析取舍、或答案随视角展开（如「综合评价 X 的架构取舍」「结合书里多个概念设计一套方案」）。
  与 pending_split 的边界：能在检索前把子查询一次列全、彼此独立、可并行 → pending_split；做不到（有依赖，或步骤集合都预定不了）→ other。
